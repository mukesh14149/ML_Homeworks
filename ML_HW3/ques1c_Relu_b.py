...
weights = [np.array([0])] + [np.random.rand(x,y) for y, x in zip(shpo[1:], shpo[:-1])]
#print len(weights),weights[0].shape,weights[1].shape,weights[2].shape,weights[3].shape

biases = [np.random.rand(1,y) for y in shpo[0:]]
...
f = open('ttclargeRelua_weight.pkl','wb')
pickle.dump(final_weight,f)

f1 = open('ttclargeRelua_bais.pkl','wb')
