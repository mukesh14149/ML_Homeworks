...
def maxout(x):
    arr=[]
    for i in x:
        arr.append(i/np.max(i))
    return np.array(arr)
    
def dermaxout(x):
    arr = []
    for i in enumerate(x):
        arr.append([0 if x <= 0 else 1 for x in i])
    return arr
...
for i in range(epoch):
	...
	for rg, rrr in enumerate(X_train):
		...
		for j in range(1,len(shpo)):
			...
			hiddenlayer_activations[j] = maxout(hidden_layer_input[j])
		...
		bb = []
		for gh in [2,1]:
			A=dermaxout(hidden_layer_input[gh]).T			
...
f = open('maxout_weight.pkl','wb')
pickle.dump(final_weight,f)

f1 = open('dermaxout_bais.pkl','wb')
